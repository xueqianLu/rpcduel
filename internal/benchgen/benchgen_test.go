...
func TestGenerate_Scenarios(t *testing.T) {
	...
	required := []string{"balance", "transaction_by_hash", "transaction_receipt",
		"block_by_number", "get_logs", "debug_trace_transaction", "debug_trace_block", "mixed_balance", "eth_call"}
...
func TestGenerate_EthCallScenario(t *testing.T) {
bf := benchgen.GenerateWithOptions(sampleDataset(), nil, benchgen.Options{})

var ec *benchgen.Scenario
for i := range bf.Scenarios {
if bf.Scenarios[i].Name == "eth_call" {
ec = &bf.Scenarios[i]
break
}
}
if ec == nil {
t.Fatal("eth_call scenario not generated by default")
}
if len(ec.Requests) == 0 {
t.Fatal("eth_call scenario has no requests")
}

// All requests must be eth_call with [callObj, blockTag] params,
// callObj must contain a "to" and "data" field, and the data must
// start with one of the known selectors.
knownSelectors := map[string]bool{
...
t.Fatalf("req %d: method=%s want eth_call", i, r.Method)
}
if len(r.Params) != 2 {
t.Fatalf("req %d: want 2 params, got %d", i, len(r.Params))
}
obj, ok := r.Params[0].(map[string]interface{})
if !ok {
t.Fatalf("req %d: param[0] not an object: %T", i, r.Params[0])
}
if _, ok := obj["to"].(string); !ok || obj["to"].(string) == "" {
t.Fatalf("req %d: missing 'to': %v", i, obj)
}
data, ok := obj["data"].(string)
if !ok || len(data) < 10 {
t.Fatalf("req %d: missing/short 'data': %v", i, obj)
}
if !knownSelectors[data[:10]] {
t.Fatalf("req %d: unknown selector %q", i, data[:10])
}
if r.Params[1] != "latest" {
t.Fatalf("req %d: blockTag=%v want latest", i, r.Params[1])
}
}

// The unique tx.To set in sampleDataset is {"0xbbb","0xaaa"} (2),
// plus balanceOf(account) for each of the 2 accounts.
if got := len(ec.Requests); got != 4 {
t.Fatalf("expected 4 eth_call requests (2 no-arg + 2 balanceOf), got %d", got)
}
}

func TestGenerate_EthCallDisabledByOnly(t *testing.T) {
// Selecting only=balance must not generate eth_call.
bf := benchgen.GenerateWithOptions(sampleDataset(), nil, benchgen.Options{
Only: map[string]bool{"balance": true},
})
for _, s := range bf.Scenarios {
if s.Name == "eth_call" {
t.Fatalf("eth_call scenario should be excluded when --only does not list it")
}
}
}
